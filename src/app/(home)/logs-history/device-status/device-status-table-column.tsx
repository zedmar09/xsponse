...
export const DeviceStatusTableColumn: ColumnDef<Details>[] = [
  {
...
<div className="text-sm font-medium text-default-600">{row.getValue("Location")}</div>
...

  {
    ...
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Connection Status" />
    ),
    cell: ({ row }) => (
      <div>
        <div
          className={`text-sm font-bold text-default-600 ${row.getValue("ConnectionStatus") == "OFFLINE" ? "text-destructive-700" : "text-green-500"} `}
        >
          {" "}
          {row.getValue("ConnectionStatus")}
        </div>
      </div>
    ),
    enableSorting: false,
...

  {
    ...
    cell: ({ row }) => (
      <div>
        <div
          className={`text-sm font-bold text-default-600 ${row.getValue("Status") == "IN ALARM" ? "text-destructive-700" : "text-default-500"} `}
        >
          {" "}
          {row.getValue("Status")}
        </div>
